...
sysbench --db-driver=mysql --test=sysbench_tests/db/update_non_index.lua --rand-type=uniform --rand-seed=$rand_seed --mysql-user=test --mysql-password=test --mysql-db=test --oltp-table-size=250000 --max-requests=0 --num-threads=8 --max-time=$1 --report-interval=1 run
