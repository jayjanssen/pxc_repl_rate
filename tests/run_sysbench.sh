...
local_pxc_index=`mysql -e "show global status like 'wsrep_local_index'" --raw 
--skip-column-names --vertical| tail -n 1`
current_unix_time=`mysql -e "select unix_timestamp()" --raw 
--skip-column-names --vertical| tail -n 1`

rand_seed=$(( ($current_unix_time / ($local_pxc_index +10 )) + 
($current_unix_time % ($local_pxc_index + 10)) ))
...
sysbench --db-driver=mysql --test=sysbench_tests/db/update_non_index.lua --rand-type=uniform --rand-seed=$rand-seed --mysql-user=test --mysql-password=test --mysql-db=test --oltp-table-size=250000 --max-requests=0 --num-threads=8 --max-time=$1 --report-interval=1 run
